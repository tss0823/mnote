...
import com.loong.mnote.common.util.SerializeUtil;
...
import com.loong.mnote.service.component.cache.JedisExecuteHandler;
import com.loong.mnote.service.component.cache.JedisService;
import com.loong.mnote.service.component.cache.QueueService;
...
//            int period = 60 * 60 * 5;  //默认过期时间
            if(!isGlobal){  //不是全部,把appName加上
                key = systemConfigComponent.getAppName() + "_"+namespace+"_"+key;
            }else{
                key = namespace+"_"+key;
            }
            if(period != null){
